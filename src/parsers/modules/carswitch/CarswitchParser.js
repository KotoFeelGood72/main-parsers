...
/**
 * Создание парсера Carswitch (функциональный стиль)
 */
function createCarswitchParser(config) {
    const parserConfig = {
    ...
    async function run() {
        ...
        try {
            ...
            for await (const listingUrl of getListings()) {
                ...
                if (data && baseParser.validateData(data)) {
                    results.push(data);
                ...
                await delay(parserConfig.delayBetweenRequests || 1000);
            }
...
module.exports = { createCarswitchParser, CarswitchParser: createCarswitchParser };
